...
public class AppPOJOUtility {
	...
	public StudentProfile getStudentProfile(IstarUser student) {
		...
		+ "("
			...
			+ "CAST ( RANK () OVER (  ORDER BY COALESCE(avg(avg_perc), 0) DESC ) AS INTEGER ) as user_rank "
			+ "from "
			+ "( "
				+ "select DISTINCT "
				...
				+ "COALESCE(percentage,0) as avg_perc "
				+ "from  "
				+ "batch_students left join leaderboard on (leaderboard.user_id = batch_students.student_id) "
				+ "where batch_students.batch_group_id in "
				+ "(select batch_group_id from batch_students where student_id = "+student.getId()+" ) "
			+ ")T1 group by student_id "
		+ ")TF where student_id = "+student.getId()+"";
