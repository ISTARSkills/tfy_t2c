...
public class AppPOJOUtility {
	...
	public StudentProfile getStudentProfile(IstarUser student) {
		if(AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {
			//ViksitLogger.logMSG(this.getClass().getName(),"POJO service");
		}
		String mediaUrlPath = "";
		...
		if (student.getUserProfile() != null) {
			if(AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {
				//ViksitLogger.logMSG(this.getClass().getName(),"FIRST NAME POJO is->" + student.getUserProfile().getFirstName());
			...
			if(student.getUserProfile().getImage()!=null && student.getUserProfile().getImage().contains("http://")){
				studentProfile.setProfileImage(student.getUserProfile().getImage());
			} else if(student.getUserProfile().getImage()!=null){
