...
public class AppUserRankUtility {
	...
	public CourseRankPOJO getLeaderboardForCourseOfUser(int istarUserId, int courseId) {
		...
		if (course != null) {
			...
			//ViksitLogger.logMSG(this.getClass().getName(),"Course Leaderboard--->"+ sql);
	...
	public List<CourseRankPOJO> getCourseRankPOJOForCoursesOfUsersBatch(Integer istarUserId) {
		...
		//ViksitLogger.logMSG(this.getClass().getName(),"Leaderboard Query->" + sql);
		SQLQuery query = session.createSQLQuery(sql);
		List<Object[]> result = query.list();
...
public CourseRankPOJO getOverAllLeaderboardForUser(int istarUserId){
	...
	try{
		...
		if(AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {		
			////ViksitLogger.logMSG(this.getClass().getName(),("findRankPoinsCoins  "+findRankPoinsCoins);
		}
		List<HashMap<String, Object>> rankData= util.executeQuery(findRankPoinsCoins);
		if(rankData.size()>0)
		{
...
/*	@SuppressWarnings("unchecked")
	...
	public StudentRankPOJO getStudentRankPOJOForCourseOfAUser(Integer istarUserId, Integer courseId){
		...
		//ViksitLogger.logMSG(this.getClass().getName(),"Student Rank pojo for course-->"+sql);
...
/*	public List<StudentRankPOJO> getStudentRankPOJOForACourseOfUsersBatch(Integer istarUserId, Integer courseId) {
		...
		for (IstarUser istarUserInBatch : allUsersOfBatch) {
			...
			for (Assessment assessment : allAssessments) {
				...
				////ViksitLogger.logMSG(this.getClass().getName(),"allUserGamifications->" + allUserGamifications.size());
				
				for (UserGamification userGamification : allUserGamifications) {
					
					////ViksitLogger.logMSG(this.getClass().getName(),"userGamification.getPoints()->"+userGamification.getPoints());
					////ViksitLogger.logMSG(this.getClass().getName(),"userGamification.getCoins()->"+userGamification.getCoins());
					
					totalPoints = totalPoints + userGamification.getPoints();
					totalCoins = totalCoins + userGamification.getCoins();
					
					//ViksitLogger.logMSG(this.getClass().getName(),"tPoints->"+totalPoints);
					//ViksitLogger.logMSG(this.getClass().getName(),"tCoins->"+totalCoins);
				}
			}
			...
			//ViksitLogger.logMSG(this.getClass().getName(),"totalCoins->"+totalCoins);
