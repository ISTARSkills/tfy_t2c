...
public class AppAssessmentServices {
		...
		if (assessment != null) {
			...
			for(SkillReportPOJO dd : shellTree)
			{
				if(AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {
					//ViksitLogger.logMSG(this.getClass().getName(),"in shell tree "+dd.getName()+" - "+dd.getId());
					//ViksitLogger.logMSG(this.getClass().getName(),"inshell tree "+" "+dd.getUserPoints()+" "+dd.getTotalPoints()+" "+dd.getPercentage());
				}
				for(SkillReportPOJO ll: dd.getSkills())
				{
					if(AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {
						//ViksitLogger.logMSG(this.getClass().getName(),"in shell tree "+ll.getName()+" - "+ll.getId());
						//ViksitLogger.logMSG(this.getClass().getName(),"inshell tree "+" "+ll.getUserPoints()+" "+ll.getTotalPoints()+" "+ll.getPercentage());
					}
				}
			}
			...
			{
				if (AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {
					////ViksitLogger.logMSG(this.getClass().getName(),("skillReportForAssesssment in data tree " + dd.getName() + " - " + dd.getId());
					////ViksitLogger.logMSG(this.getClass().getName(),("skillReportForAssesssment data tree " + " " + dd.getUserPoints() + " "
				...
				for (SkillReportPOJO ll : dd.getSkills()) {
					...
					//	System.err
						...
						////ViksitLogger.logMSG(this.getClass().getName(),("skillReportForAssesssment in data tree " + " " + ll.getUserPoints() + " "
			...
			//ViksitLogger.logMSG(this.getClass().getName(),"countOfStudentInBatchAndAttended"+countOfStudentInBatchAndAttended);
			...
			//ViksitLogger.logMSG(this.getClass().getName(),"questionsSQL->"+questionsSQL);
	...
	private List<SkillReportPOJO> fillShellTreeWithAssessmentData(List<SkillReportPOJO> shellTree, int istarUserId, Assessment assessment) {
		...
		//ViksitLogger.logMSG(this.getClass().getName(),"getDataForTree in assessment "+getDataForTree);
		DBUTILS util = new DBUTILS();
			...
			for(SkillReportPOJO mod : shellTree)
			{
				if(mod.getId() == moduleId)
				{
					...
					for(SkillReportPOJO cmsSkill: cmsSkills)
					{
						...
						{
							if(AppProperies.getProperty("serverConfig").equalsIgnoreCase("dev")) {
							////ViksitLogger.logMSG(this.getClass().getName(),"idsss<<<<<<<<<<<<<<<<<"+cmsSkill.getId());
							}
							
							if(cmsSkill.getId()==188)
							{
								////ViksitLogger.logMSG(this.getClass().getName(),"Math.ceil(userPoints)<<<<<<<<<<<<<<<<<"+Math.ceil(userPoints));
							}
														
							if(cmsSkill.getAccessedFirstTime()==true)
							{
								////ViksitLogger.logMSG(this.getClass().getName(),(cmsSkill.getId()+" is accessed for the first time"+cmsSkill.getAccessedFirstTime());
	...
	private List<SkillReportPOJO> getShellTreeForAssessment(int assessmentId ) {
		...
		//ViksitLogger.logMSG(this.getClass().getName(),("getShellTreeForAssessment>>>"+shelltreeData);
...
/*	public AssessmentReportPOJO getAssessmentReport(int istarUserId, int assessmentId) {
		//ViksitLogger.logMSG(this.getClass().getName(),"Getting assessment report");
		AssessmentReportPOJO assessmentReportPOJO = null;
	...
	public List<Integer> getAttemptedAssessmentsOfUser(int istarUserId){
		...
		//ViksitLogger.logMSG(this.getClass().getName(),"all Assessments of user from UsrGmfctn---->"+sql);
	 ...
	 * 
	 * //ViksitLogger.logMSG(this.getClass().getName(),"maxPointsOfAllUsers->" + maxPointsOfAllUsers);
	 * //ViksitLogger.logMSG(this.getClass().getName(),"maxPointsOfAssessment->" + maxPointsOfAssessment);
	 * 
